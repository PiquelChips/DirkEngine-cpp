...
#define IM_VEC2_CLASS_EXTRA                                                     \
        constexpr ImVec2(const vk::Extent2D& f) : x(static_cast<float>(f.width)), y(static_cast<float>(f.height)) {}                   \
        operator vk::Extent2D() const { return vk::Extent2D(static_cast<std::uint32_t>(x),static_cast<std::uint32_t>(y)); } \
